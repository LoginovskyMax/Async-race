...
export default function MainPage({ setPageNumber, pageNumber, changeWinners, winners }:IProps) {
  const [carsArr, setCarsArr] = useState<Icar[]>([]);
  const [carObj, setCarObj] = useState<Icar>({ name:'', color:'#FFFFFF', id:-1 });
  ...
  const DeleteCar = (id:number) => {
    fetch(`http://localhost:3000/garage/${id}`,
      { method:'DELETE' })
      .then(response => {
        if (response.status === 200) {getCars(pageNumber);}
      });
  ...
  const getCars = (page:number) => {
    fetch(`http://localhost:3000/garage?_page=${page}&_limit=7`)
      ...
      if (response.headers.get('X-Total-Count') !== null) {
        ...
        if ((Math.ceil(Number(response.headers.get('X-Total-Count')) / 7)) !== 0) {
          setTotalPages(Math.ceil(Number(response.headers.get('X-Total-Count')) / 7));
        }
      }
  ...
  const fetchChangedCar = (obj:{ name:string, color:string }, id:number | undefined) => {
    fetch(`http://localhost:3000/garage/${id}`,
      { method:'PUT',
        headers:{ 'Content-Type': 'application/json' },
        body: JSON.stringify(obj),
  ...
  const create100Cars = () => {
    for (let i = 0;i < 100;i++) {
      ...
      const secondName = firstName[1][Math.floor(Math.random() * firstName[1].length)];
  ...
  return (
    <div>
      <FormComp propFunc={addCar}/>
      <FormComp propFunc={fetchChangedCar}
	changeObj={carObj}
	setCarObj={setCarObj}
	save={save.current}/>
      <div className='btn-block'>
        <p className='garage-text'>Cars in garage: {totalCars}</p>
        <button onClick={()=>setStartRace(true)}
	className='btn'
	disabled={startRace}>Start race</button>
        <button onClick={stopRace}
	className='btn'
	disabled={!endRace}>Reset</button>
        <button onClick={create100Cars} className='btn'>Create 100 cars</button>
      </div>
      {carsArr.map((item) => <CarComp carData={item} 
	ChangeCar={ChangeCar}
	DeleteCar = {DeleteCar}
	getCars={getCars}
	startRace = {startRace}
	...
	key={item.id}/> )}
      {emptyGarageMessage && <p>Your garage is empty, create some cars</p>}
      {winnerData.name && <p className='winner-text'>Winner {winnerData.name} with time {winnerData.time / 1000}s</p>}
      <PagesSelector pageNumber={pageNumber}
	totalPages={totalPages}
	ChangePage={ChangePage}/>
      {serverError && <ModalWindow />}
    </div>
  );
}

...
export default function CarComp({ carData, ChangeCar, startRace, createWinner, DeleteCar }:Props) {
  ...
  const StartEngine = async (status:string) => {
    ...
    const response = await fetch(`http://localhost:3000/engine?id=${carData.id}&status=${status}`,
      { method:'PATCH' });
  ...
  return (
    <div className='car-component'>
      <div className='car-component__image-block'>
           <SvgImage color = {carData.color} 
	animationTime = {animationTime}
	engineBroke = {engineBroke}
	startEngine = {engineStarted}
	width = {100}/>
           <img src={FlagImg} alt='flag' width='30' className='image-block__flag'/>
       </div>
     <p>{carData.name}</p>
     <div className='btns-block'>
       <button onClick={()=>StartEngine('started')}
	className='btn'
	disabled= {(engineStarted)}>Start</button>
       <button onClick={()=>StartEngine('drive')}
	className='btn'
	disabled= {driveMode || !engineStarted}>Drive</button>
       <button onClick={()=>StartEngine('stopped')}
	className='btn'
	disabled= {!engineStarted}>Stop</button>
     <button onClick={()=>ChangeCar(carData)} className='btn'>Change</button>
         <button onClick={()=>DeleteCar(carData.id) } className='btn'>Delete</button>
     </div>
    </div>
  );
}

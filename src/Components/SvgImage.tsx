...
export default function SvgImage({ color, animationTime, engineBroke, width, startEngine }:Props) {
  ...
  useEffect(()=>{
    myreq = requestAnimationFrame(move);
    if (engineBroke) {cancelAnimationFrame(myreq);}
  ...
  return (
	...
	style={{ fill:color,
	  width:`${width}px`,
	  transform:(animationTime === 0) ? 'translate(0px) rotateY(180deg)' : undefined }}
          >
        <img src={Smoke} alt="smoke" width='30' className='engineSmoke' style={{ display:(engineBroke ? 'block' : 'none') }}/>
	...
	viewBox="0 0 125 40">
        ...
        <img src={Smoke} alt="smoke" width='20' className='gasSmoke' style={{ display:(startEngine ? 'block' : 'none') }}/>
    </div>
  );
}

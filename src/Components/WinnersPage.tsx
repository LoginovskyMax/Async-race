...
let allCars:Icar[] = [];
export default function WinnersPage({ getWinners, winners, winPage, sortBy, sortOrder, totalCars, totalPages }:Iprops) {
  ...
  return (
    <div className='winner-page'>
        ...
        <p className='winners-item__text clicked' onClick={()=>sortingFn('id')} >ID { sortOrder[0] && sortBy[0].name === 'id' ? <span>&#8659;</span> :  <span>&#8657;</span>}</p>
        <p className='winners-item__text'>Name</p>
        <p className='winners-item__text'>Car</p>
        <p className='winners-item__text clicked' onClick={()=>sortingFn('wins')}>Wins { sortOrder[0] && sortBy[0].name === 'wins' ? <span>&#8659;</span> :  <span>&#8657;</span>}</p>
        <p className='winners-item__text clicked' onClick={()=>sortingFn('time')}>BestTime { sortOrder[0] && sortBy[0].name === 'time' ? <span>&#8659;</span> :  <span>&#8657;</span>}</p>
        </div>
        {mixedArr.map((item) => 
        <div key={item.id} className='winners-item'>
          <p className='winners-item__text'>{item.id}</p>
          <p className='winners-item__text'>{item.name}</p>
          <SvgImage color ={item.color}
	animationTime = {0}
	engineBroke = {false}
	startEngine = {false}
	width = {60}/>
          ...
          <PagesSelector pageNumber={winPage[0]}

...
export default function FormComp({ propFunc, changeObj, setCarObj, save }:IProps) {
  ...
  const SaveToLS = () => {
    ...
    localStorage.setItem('changeInpLoginovskiy', JSON.stringify(obj));
    } else {
      localStorage.setItem('createInpLoginovskiy', JSON.stringify(obj));
    }
  };
  ...
  const DeleteLs = ()=>{
    if (changeObj !== undefined) {
      ...
      localStorage.removeItem('createInpLoginovskiy');
    }
  };
  ...
  useEffect(() => {
    if (changeObj !== undefined) {
      ...
      if (localStorage.getItem('changeInpLoginovskiy')) {
        const data = JSON.parse((localStorage.getItem('changeInpLoginovskiy') as string));
        setCarColor(data.carColor);
        setCarName(data.carName);
    }
  } else {
    if (localStorage.getItem('createInpLoginovskiy')) {
      const data = JSON.parse((localStorage.getItem('createInpLoginovskiy') as string));
      setCarColor(data.carColor);
      setCarName(data.carName);
     }
  }
  ...
  return (
	...
	disabled = {changeObj !== undefined  ? carName !== '' && changeObj.id !== -1 ? false : true : false}
	onClick={()=>{
    validateData();
    DeleteLs();
}}>{changeObj ? 'Change' : 'Create'}</button>
     </form>
  );
}

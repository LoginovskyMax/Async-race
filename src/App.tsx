...
function App() {
  ...
  const sortBy = useState<{ name:string, sort:string }>({ name:'id', sort:'ASC' });
    ...
    fetch(`http://localhost:3000/winners${queryParams}`)
      .then<IWin[]>(response => {
        if (response.headers.get('X-Total-Count') !== null) {
          ...
          if ((Math.ceil(Number(response.headers.get('X-Total-Count')) / 10)) !== 0) {
            setTotalPages(Math.ceil(Number(response.headers.get('X-Total-Count')) / 10));
          }
        }
  ...
  return (
    <div className="App">
      <header className="App-header">
        <button onClick={()=>navigate('/')} className='btn'>Your Garage</button>
        <button onClick={()=>navigate('/winners')} className='btn'>Winners page</button>
      </header>
      <Routes>
        <Route path='/' element={<MainPage pageNumber={pageNumber}
	setPageNumber={setPageNumber}
	changeWinners={changeWinners}
	winners={winners}/>}/>
        <Route path='/winners' element={<WinnersPage  getWinners = {getWinners}
	winners = {winners}
	winPage = {winPage}   
	sortBy = {sortBy}  
	sortOrder = {sortOrder}
